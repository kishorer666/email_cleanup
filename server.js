...
const CLIENT_ID = '1047657398962-2ep44v9tajpfs5c7a7q3j64e9ivgu7r9.apps.googleusercontent.com';
const CLIENT_SECRET = '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';
...
app.get('/auth/google/callback', async (req, res) => {
  try {
    ...
    console.log('OAuth tokens received - has refresh token:', !!tokens.refresh_token);
    res.redirect('/');
  } catch (err) {
...
async function requireAuth(req, res, next) {
  if (!req.session || !req.session.tokens) return res.status(401).json({ error: 'not-authenticated', message: 'Please log in to continue.' });
  try {
    // Check if token needs refresh
    const oauth2Client = createOAuthClient();
    oauth2Client.setCredentials(req.session.tokens);
    const now = Date.now();
    if (req.session.tokens.expiry_date && req.session.tokens.expiry_date <= now + 60000) {
      // Token expires soon or already expired, refresh it
      const { credentials } = await oauth2Client.refreshAccessToken();
      req.session.tokens = credentials;
    }
  } catch (err) {
    console.error('Token refresh failed:', err && err.message);
    return res.status(401).json({ error: 'token-refresh-failed', message: 'Your session has expired. Please log out and log in again to continue.' });
  }
  next();
}
...
app.post('/api/gmail/search', requireAuth, async (req, res) => {
  ...
  try {
    ...
    let errorMsg = err.message || 'Search failed';
    let errorCode = 'search-failed';

    if (errorMsg.includes('No refresh token') || errorMsg.includes('invalid_grant')) {
      errorMsg = 'Your authentication has expired. Please log out and log in again.';
      errorCode = 'auth-expired';
      return res.status(401).json({ error: errorCode, message: errorMsg });
    }
    if (errorMsg.includes('quota')) {
      errorMsg = 'Gmail API quota exceeded. Please try again later.';
      errorCode = 'quota-exceeded';
      return res.status(429).json({ error: errorCode, message: errorMsg });
    }
    res.status(500).json({ error: errorCode, message: errorMsg });
  }
});
...
app.post('/api/gmail/delete', requireAuth, async (req, res) => {
  ...
  try {
    ...
    let errorMsg = err.message || 'Delete failed';
    let errorCode = 'delete-failed';
